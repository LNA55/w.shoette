# What works for me — Cahier des charges (v7)

> **Version 7 — 2 juillet 2026 à 16h19.** Mise au net de session. Ajout depuis la v6 :
> **Écran Réglages — les 4 parties racine (I. Design · II. Moteur · III. Gestion des données personnelles · IV. Informations légales) deviennent repliables et FERMÉES par défaut** (`<details class="set-part-block">`, titre en `<summary>` + chevron). L'écran s'ouvre en vue compacte. L'état ouvert/fermé de chaque partie est **conservé le temps de la session** (`S.partsOpen`, écouteur `toggle` en capture) pour survivre aux re-renders déclenchés par un réglage interne, mais **repart replié** au prochain lancement (non persisté). Détail en §12. Reprise du statut v6 ci-dessous.
>
...
(niveau 2) et des **sections repliables** (niveau 3). **v7 (02/07/2026) : les 4 parties racine
elles-mêmes sont désormais repliables et FERMÉES par défaut** (`<details class="set-part-block">`,
titre en `<summary>` + chevron) — l'écran s'ouvre en vue compacte ; l'état ouvert/fermé de
chaque partie est **conservé le temps de la session** (`S.partsOpen`, écouteur `toggle` en
capture) pour survivre aux re-renders déclenchés par un réglage interne, mais **repart replié**
au prochain lancement de l'app (non persisté au store). Hiérarchie visuelle : partie en gras +
